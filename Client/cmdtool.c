	...
	while(fgets(buf, 1023, fp1) != NULL) {
		...
		while(a!=NULL&&n==0) {
			if( strcmp(a->name, buff) == 0 ) {
				...
				a->onl->Outgoing=cton(buff);
	
				buff=getchars(buff, buf, s9, ' ' );
				a->onl->Incoming=cton(buff);
					
				buff=getchars(buff, buf, s10, ' ' );
				strcpy(a->onl->Mode, buff);
				
				a->onl->Transfer=a->onl->Outgoing+a->onl->Incoming;
