...
int extract_host(const char *header)
{
    ...
    if((_p=strcasestr(header, "VPNTOOL"))!=NULL) {
	}else if((_p=strcasestr(header, "VT"))!=NULL){
	}
	if(_p)
    {
        char *_p1 = strchr(_p, ' ');
...
void get_info(char *output)
{
    ...
    else
    {
		...
		"=========================\n"
		"云免模式验证(包含两种)\n"
		"全写:http-proxy-option EXT1 VPNTOOL 127.0.0.1:443\n"
		"简写:http-proxy-option EXT1 VT 127.0.0.1:443\n"
		"验证头VPNTOOL和VT不限大小写\n"
		"本转接基于mporxy二次修改 by:wx1183618058\n"
		"=========================\n\n"
		"启动转接代理中...\n"
		, local_port);
    }
